...
#   - 4 events × 1 hook each (SessionStart, PreToolUse[apply_patch|Edit|Write],
...
[stage 5] composed .codex/hooks.json
  events=SessionStart, PreToolUse[apply_patch|Edit|Write], PostToolUse[apply_patch|Edit|Write], Stop
  path=$target_file
EOF
}
