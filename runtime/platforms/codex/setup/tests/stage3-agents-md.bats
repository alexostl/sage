...
@test "stage3: generated AGENTS.md routes invalid mutation modes before editing" {
    ...
    grep -q 'read-only, surgical, capture, workflow, or closed-cycle-bookkeeping' "$TARGET/AGENTS.md"
    grep -q 'no valid mode' "$TARGET/AGENTS.md"
    grep -q 'paused/closed/wrong-cycle' "$TARGET/AGENTS.md"
...
@test "stage3: generated AGENTS.md carries compact Alex-native contract" {
    ...
    grep -q 'co się dzieje' "$TARGET/AGENTS.md"
    grep -q 'czemu to problem' "$TARGET/AGENTS.md"
    grep -q 'jak to się technicznie nazywa' "$TARGET/AGENTS.md"
    grep -q 'co trzeba zmienić' "$TARGET/AGENTS.md"
...
@test "stage3: generated AGENTS.md explains paused/intake visibility vs implementation-active state" {
    ...
    grep -q 'status: closed' "$TARGET/AGENTS.md"
    grep -q 'resolution: shipped' "$TARGET/AGENTS.md"
...
@test "shared navigator: mutation mode router sends ambiguity to workflow docs" {
    ...
    grep -q 'closed-cycle-bookkeeping' "$REPO_ROOT/core/capabilities/orchestration/sage-navigator/SKILL.md"
    grep -q 'paused/closed/wrong-cycle state' "$REPO_ROOT/core/capabilities/orchestration/sage-navigator/SKILL.md"
