...
4. `origin/selfhost` as the active self-host work surface and the
...
a PR on GitHub web, manually re-pick `selfhost` as the base — the
...
For this repo, this checkout is reserved for `selfhost`. `main` is remote-only
for normal work and exists as `origin/main` / `upstream/main`. `codex-port`
keeps its own separate worktree because it is a distinct integration surface.
