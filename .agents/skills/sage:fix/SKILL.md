...
name: sage:fix
description: >-
  Bugfix workflow. Use for regressions, breakages, and debugging with root-cause-first discipline.
---
...
- SCOPE THE FIX after root cause is confirmed:
  ...
  Systemic (5+ files, interface changes) → ESCALATE to $sage:build or $sage:architect.
...
[1] {finding} — {severity} — /sage:fix (mechanical)
[2] {finding} — {severity} — /sage:fix (mechanical)
...
[N] New session — type /sage:fix to continue
...
[1] Escalate to /sage:build — write spec for the fix as a feature
[2] Escalate to /sage:architect — the root cause is architectural
...
[E] Escalate — type /sage:build or /sage:architect instead
[N] New session — type /sage:fix to continue
...
**Verification artifact (Moderate+ scope):** Write
...
out-of-shape verification files. The "Pre-fix reproducer" section is
where the failing test or failing repro lands; the "Test command +
pasted output" section MUST contain a fenced block with real output,
not a prose summary.
...
Next steps:
  /sage:reflect — review what went wrong, prevent recurrence
  /review  — independent evaluation of the fix
  /sage:build   — spec → plan → implement (if fix revealed a feature need)
...
- If the fix reveals a systemic issue, ESCALATE. Do not scope-creep
  a fix into a rebuild. Offer /sage:build or /sage:architect.
